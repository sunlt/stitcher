...
function Package() {
	...
	if (!path.existsSync(this.options.ignoreFile)) return;
...
Package.prototype = {
	...
	create:function (){
		...
		var zip = new AdmZip(path.join(__dirname,'../assets/project_template.zip/'));
		zip.extractAllTo(path.join('./',projectName),true);
	},
	exec: function() {
