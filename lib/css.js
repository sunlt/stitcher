...
Package.prototype = {
	...
	compile: function(file, callback,build) {
		...
		p.done(function(rev,cssPath, lessPath) {
			...
			fs.readFile(filePath, 'utf-8', function(err, data) {
				...
				var imagesPath = '';
				if(!build)
					imagesPath = path.relative(path.dirname(filePath),util.expand(this.options.ouputImagesFolder));
				else
					imagesPath = path.relative('/css',this.options.ouputImagesFolder);

				imagesPath = imagesPath.replace(/\\/g,'/');

				var lessText = require('util').format("@images_path:'%s';\n%s",imagesPath,data);
