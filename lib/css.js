...
Package.prototype = {
	...
	build: function() {
		...
		p.done(function (rev){
			...
			var css = source.join('').replace(/url\s*\(["'\s]?(\s*[A-Za-z0-9\-\_\.\/\:]+\s*)["'\s]?\)\s*/gi, function(p,url) {
			  return require('util').format(' url(../%s) ', util.expand(url));
			});

			var css = require('./cssmin').cssmin(css);
